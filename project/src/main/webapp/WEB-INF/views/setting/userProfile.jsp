...
#btnUpdateImg:hover {
	...
	<div class="container-fluid">
		<div id="content">
			...
			<!-- 프로필 이미지 변경 영역 -->
			<div id="right">
				 <div style="height: 70px; margin-left :30px; margin-right:30px; background-color: #F3F1F5; text-align: center; " class="d-flex align-items-center justify-content-between rounded-4  m-2 ps-3 pe-4">
       				 <div  style="font-size:18px;font-weight: bold; max-width:180px; margin-left:10px;" class="px-7">프로필 사진</div>
    
                  </div>
					<div class="my-2 card" style="border:none; padding-top:30px; display: flex; justify-content: center; align-items: center;">
				...
				<form action="${updateImgPage}" method="post" enctype="multipart/form-data" >
				  		  ...
				  		  <input class="form-control" style="margin-top:20px;" type="file"  id="profile" name="profile" accept="image/*" onchange="setThumbnail(event);">
				...
				<form action="${settingBasicImg}"  >	
					<input type="hidden" name="user_id" id="user_id" value="${user.user_id}">
					<button type="submit" class="btn btn-secondary" style=" font-size:13px; font-weight:bold; float:right; border:none; background-color:#610C9F;">기본 변경</button>
